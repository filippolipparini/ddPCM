...
\begin{wideitemize}
\item A {\bf hollow cavity} $\Omega$ bounded by a surface $\Gamma$ accommodates {\bf solute}
...
%\item {\bf Electrostatic energy} of solute is {\bf modified} by presence of solvent
\item Solute/solvent {\bf interaction} contributes {\bf extra term} $E_s$ to {\bf electrostatic energy} of solute \emph{in vacuo}
%\item Add {\bf contribution} $E_s$ to electrostatic energy \emph{in vacuo}
...
\item Operator $L$ is {\bf well-conditioned}
...
\begin{wideitemize}
\item State-of-the-art: {\bf Continuous Surface Charge} (CSC) method by York and Karplus
\item CSC produces {\bf smooth energy} profiles
\item CSC is {\bf not} systematically {\bf improvable}, and {\bf poorly} conditioned
\item Employ \textsc{Gaussian}\textsuperscript{\textregistered} for {\bf benchmark} tests.
\end{wideitemize}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.18]{figures/crambin.png}
\caption{\scriptsize Crambin, 642 atoms}
\end{center}
\end{figure}

\begin{wideitemize}
\item Extract {\bf chains} of atoms of length 80, 160, 240, etc. to employ as {\bf benchmark} set.
\end{wideitemize}
