...
\right.
\end{equation}
Here $\Gamma=\partial\Omega$ is the boundary of the cavity, $\partial_\nu$ is the derivative with respect to the outer normal on $\Gamma$, and $[\![\,\cdot\,]\!]$ is the jump operator (inside minus outside) on $\Gamma$.
...
\end{equation}
where $g(\varepsilon_s) = (\varepsilon_s+1)/(\varepsilon_s-1)$, $\cI$ is the identity and $\cD$ is the double layer operator, also relative to $\Gamma$, it can be shown\cite{ReviewPCM_2005} that the apparent surface charge satisfies
\begin{equation}
\label{eq:IEFPCM}
...
\]
for some unknown coefficients $X = [X_j]_\ell^m$. The choice of this ansatz implies a spectral-like numerical method. The scaling factor $1/r_j$ has been introduced for convenience, as seen from the derivation in Appendix \ref{app:cosmo}. We approximate $\Phi_{\varepsilon,j}$ and $\Phi_j$ in the same fashion, namely
...
Since operators $A_\varepsilon$ and $L$ are known in closed form, it is immediate to conclude that they depend upon the nuclear positions. The right-hand-side $F$, see Appendix \ref{app:pcm}, also depends upon the nuclear positions through both the characteristic function and the potential.  Those dependencies upon the nuclear positions are needed in the computation of the ddPCM-forces, discussed in the following Section.
